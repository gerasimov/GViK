...
(function() {
    ...
    function find(artist, title, arr, dur, callback, opt) {
        ...
        var audioMap = searchInArray(arr, [artist, title, dur], _eq);

        if (!audioMap.length) {
            if (!(audioMap = searchInArray(arr, [artist, title.dur], _eq2)).length) {
                return callback(null);
            }
        }

        audioMap = gvik.core.filter(audioMap, function(a) {
                return a.dur === 0;
            })
            .sort(function(a, b) {
